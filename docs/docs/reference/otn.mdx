...
- Device types and platforms for optical gear. Model the physical asset as a DcimPhysicalDevice and link it with OtnGenericDevice.dcim_device. That edge is also the route to a rack elevation, since the physical record carries position, rack_face and a location that may be a LocationRack. Nothing keeps OtnGenericDevice.site in agreement with the site above that rack.
