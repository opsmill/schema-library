...
- Validation of what a schema cannot express, such as which port kind may hold a pluggable, or whether a mux client port binds a channel on the same plan its multiplexer declares. This extension ships no checks; the schema comments mark each such rule.
  ...
  - name: band
    ...
    computed_attribute:
      ...
      jinja2_template: '{% if center_wavelength_nm__value is not none %}{% if center_wavelength_nm__value
        ...
        < 1530 %}S{% elif center_wavelength_nm__value < 1565 %}C{% else %}L{% endif
        %}{% endif %}'
    optional: true
